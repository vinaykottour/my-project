...
CategoryToolsCloud ProviderAWS (EC2, EKS, Load Balancer, IAM)Web ServerNginxContainerizationDocker, Docker HubOrchestrationKubernetes (AWS EKS)CI/CDGitHub ActionsMonitoringPrometheus, GrafanaInfrastructure as CodeTerraformVersion ControlGit, GitHubOSUbuntu (EC2 instances)
...
EnvironmentBranchDeploys ToTriggerDevdevDev EC2 (Docker container)Push to devStagingstagingStaging EC2 (Docker container)Push to stagingProdmainAWS EKS (Kubernetes cluster)Push to main
...
bashkubectl get nodes                          # list worker nodes
...
bashdocker stop portfolio && docker rm portfolio
...
bashhelm install monitoring prometheus-community/kube-prometheus-stack --namespace monitoring
...
bashkubectl create secret generic portfolio-secret \
...
bashterraform init      # set up working directory and provider plugins
